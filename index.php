...
<head>
    ...
    $cfg_bg = 'https://images.unsplash.com/photo-1557683316-973673baf926?q=80&w=2029';
    $cfg_text = '#000000';
    try {
        $pdo = getPDO();
        $stmt = $pdo->query('SELECT bg_image, text_color FROM config ORDER BY id DESC LIMIT 1');
        $row = $stmt->fetch();
        if ($row) {
            if (!empty($row['bg_image'])) { $cfg_bg = $row['bg_image']; }
            if (!empty($row['text_color'])) { $cfg_text = $row['text_color']; }
        }
    } catch (Throwable $e) {
        // Giữ giá trị mặc định nếu DB chưa sẵn sàng
    }
    ?>
    <style>
        body.client-page {
            /* Nhận link ảnh từ Server */
            background-image: url('<?php echo htmlspecialchars($cfg_bg, ENT_QUOTES); ?>') !important;
        }
        ...
        .glass-container h3 {
            color: <?php echo htmlspecialchars($cfg_text, ENT_QUOTES); ?> !important;
        }
